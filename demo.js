...
const startCells = new Map(crossword.grid.words.map(({ x, y, index }) => [`${x}:${y}`, index]))

const grid = Array.from(crossword.grid.cells.values())
  .map((cell) => `<div class="cell${crossword.grid.blanks.includes(cell) ? ' blank' : ''}">${startCells.get(cell + []) ? '<sup>' + startCells.get(cell + []) + '</sup>' : ''}</div>`)
  .join('')

el.innerHTML = `<div id="grid" style="grid-template-columns:repeat(${crossword.grid.width}, 30px);grid-template-rows:repeat(${crossword.grid.height}, 30px)">${grid}</div>`
